...
def create_mnist_dataloaders(root, batch_size, p_missing, train_val_split=0.9, num_workers=0, download=True, **kwargs):
    ...
    dataset = datasets.MNIST(root=root, train=False, transform=transform, download=download)
    ...
    class DictWrapper:
        ...
        def __next__(self):
            ...
            if isinstance(x, list) or isinstance(x, tuple):
                 return {"x_clean": x[0]} # if loader returns [x, y] but wait, dataset returns (x, y)
            return {"x_clean": x}
...
def subs_argmax(model, xt, logits):
    """
    Helper to perform SUBS inference (argmax) given logits and current state xt.
    """
    ...
    unmasked = (xt != mask_id)
    if unmasked.any():
        target = xt.squeeze(1).long()
        
        # Create a tensor with -inf everywhere except 0 at the target class
        forced = torch.full_like(log_probs, fill_value=neg_inf)
        
        forced_flat = forced.permute(0, 2, 3, 1).contiguous().view(-1, C)
        target_flat = target.view(-1)
        row_idx = torch.arange(forced_flat.shape[0], device=logits.device)
        
        forced_flat[row_idx, target_flat] = 0.0
        
        forced = forced_flat.view(B, H, W, C).permute(0, 3, 1, 2)
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/mdm_baseline.yaml", help="Path to config file")
    ...
    sample_path = os.path.join(args.output_dir, f"epoch{epoch:03d}_samples_uncond.png")
    vutils.save_image(samples, sample_path, nrow=args.num_samples//2, normalize=False)
    print(f"Saved samples to {sample_path}")
    
    # 2. Reconstruction
    print("Generating reconstructions...")
    data_cfg = cfg_dict.get("data", {})
    # Use a small batch for viz
    ...
    t_mid = torch.full((x_clean.shape[0],), t_mid_val, device=device, dtype=torch.long)
    
    # Forward mask
    xt_mid, mask_mid = model.schedule.forward_mask(x_clean, t_mid)
    
    # Encode
    xt_one_hot = model._encode_states(xt_mid)
    logits = model.unet(xt_one_hot, t_mid)
    
    # Inference
    recon_mid = subs_argmax(model, xt_mid, logits)
    
    # Visualization
    # x_clean | xt_mid (gray for mask) | recon_mid
    
    xt_vis = xt_mid.float()
    xt_vis[xt_mid == 2] = 0.5
    
    comparison = torch.cat([x_clean, xt_vis, recon_mid], dim=0)
    # nrow=args.num_samples will make 3 rows: Clean, Masked, Recon
    recon_path = os.path.join(args.output_dir, f"epoch{epoch:03d}_recon_t{t_mid_val}.png")
    vutils.save_image(comparison, recon_path, nrow=args.num_samples, normalize=False)
